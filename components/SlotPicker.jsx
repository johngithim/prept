  ...
  return (
    <>
      ...
      <div className={"flex flex-col gap-4"}>
        <div
          className={
            "bg-[#0f0f11] border border-white/10 rounded-2xl p-7 flex flex-col gap-6"
          }
        >
          <div className={"flex items-start justify-between gap-3"}>
            ...
            <div className={"text-right shrink-0"}>
              ...
              <p
                className={
                  "font-serif text-2xl leading-none bg-linear-to-br from-amber-300 to-amber-500 bg-clip-text text-transparent"
                }
              >
                {interviewerCredits}{" "}
          ...
          <div
            className={
              "flex gap-2 overflow-x-auto pb-0.5 scrollbar-none -mx-1 px-1"
            }
          >
            {dates.map((date) => {
              ...
              {slots.map((slot) => {
                ...
                return (
                  <button
                    ...
                    className={`relative px-3.5 py-2.5 rounded-xl border text-xs transition-all duration-200 ${isSelected ? "border-amber-400/40 bg-amber-400/10 text-amber-400 shadow-[0_0_12px_rgba(251,191,36,0.08)]" : slot.isBooked ? "border-white/5 bg-white/20 text-stone-700 cursor-not-allowed" : "border-white/10 text-stone-500 hover:border-white/20 hover:text-stone-400"}`}
                  >
                    {formatTime(slot.startTime)}
                    {slot.isBooked && (
                      <span
                        className={
                          "absolute inset-x-0 bottom-0.5 text-center text-stone-700 leading-none"
                        }
                        style={{ fontSize: "9px" }}
                      >
                        booked
